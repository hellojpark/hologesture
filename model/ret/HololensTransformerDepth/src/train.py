...
# # wandb.init(project = 'Hololens', name = 'cnn_transformer')
...
class GestureTrainer(object):
    ...
    def __init__(self, configer):
        ...
        self.crop_data_path = configer.get("data", "crop_data_path")
    ...
    def init_model(self):
        ...
        self.train_loader = DataLoader(
            Dataset(self.configer, self.normal_type, self.specific_path, self.width, self.height, self.data_path, self.crop_data_path, split="train", data_type=self.data_type,
                    n_frames=self.clip_length, optical_flow=self.optical_flow),
        ...
        self.val_loader = DataLoader(
            Dataset(self.configer, self.normal_type, self.specific_path, self.width, self.height, self.data_path, self.crop_data_path, split="val", data_type=self.data_type,
                    n_frames=self.clip_length, optical_flow=self.optical_flow),
        ...
        if self.dataset == "hololens":
            ...
            self.test_loader = DataLoader(
                Dataset(self.configer, self.specific_path, self.width, self.height, self.data_path, self.crop_data_path, split="test", data_type=self.data_type,
                        transforms=self.val_transforms, n_frames=self.clip_length, optical_flow=self.optical_flow),
                batch_size=1, shuffle=False, drop_last=True,
                num_workers=self.configer.get('solver', 'workers'), pin_memory=True, worker_init_fn=worker_init_fn)
            
        # wandb.init(project="Depth tran normal setting", name="no aug video-normal bare depth")
        wandb.login()
        wandb.init(project="save all random approach check", name="50 epoch randomseed frame-normal {} {}_noaug {}".format(self.hand_type, self.specific_path, self.number))
    ...
    def __val(self):
        ...
        _ = self.model_utility.save(accuracy, self.net, self.optimizer, self.iters, self.epoch + 1)
        if ret < 0:
            return -1
