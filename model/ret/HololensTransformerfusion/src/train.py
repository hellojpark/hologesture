...
class GestureTrainer(object):
    ...
    def init_model(self):
        ...
        self.train_loader = DataLoader(
            Dataset(self.configer, self.rgb_normal_type, self.depth_normal_type, self.rgb_specific_path, self.depth_specific_path, self.rgb_width, self.rgb_height, self.depth_width, self.depth_height,
                    self.data_path, self.crop_path, split="train", data_type=self.data_type, n_frames=self.clip_length, optical_flow=self.optical_flow),
        ...
        self.val_loader = DataLoader(
            Dataset(self.configer, self.rgb_normal_type, self.depth_normal_type, self.rgb_specific_path, self.depth_specific_path, self.rgb_width, self.rgb_height, self.depth_width, self.depth_height,
                    self.data_path, self.crop_path, split="val", data_type=self.data_type, n_frames=self.clip_length, optical_flow=self.optical_flow),
        ...
        if self.dataset == "hololens":
            ...
            self.test_loader = DataLoader(
                Dataset(self.configer, self.normal_type, self.rgb_specific_path, self.depth_specific_path, self.rgb_width, self.rgb_height, self.depth_width, self.depth_height,
                        self.data_path, self.crop_path, split="test", data_type=self.data_type, transforms=self.val_transforms, n_frames=self.clip_length, optical_flow=self.optical_flow),
                batch_size=1, shuffle=False, drop_last=True,
                num_workers=self.configer.get('solver', 'workers'), pin_memory=True, worker_init_fn=worker_init_fn)
            
        wandb.login()
        wandb.init(project="save all random approach check", name="50epoch randomseed no aug tran rgbbaseddepth+rgb frame-normal late fusion {}".format(self.configer.get("number")))

    def __train(self):
        """Train function for every epoch."""

        self.net.train()
        for rgb, depth, rgb_label, depth_label in tqdm(self.train_loader, desc="Train"):
            """
            input, gt
            """
            ...
            self.iters += 1
            self.update_metrics("train", loss.item(), rgb_inputs.size(0),
                                float((predicted==correct).sum()) / len(correct))
        accuracy = self.accuracy["train"].avg
        print(f'train avg accuracy : {accuracy}')
        wandb.log({
    ...
    def __val(self):
        ...
        with torch.no_grad():
            ...
            for rgb, depth, rgb_label, depth_label in tqdm(self.val_loader, desc="Val", postfix=""+str(np.random.randint(200))):
                """
                input, gt
                """
                rgb_inputs = rgb.to(self.device)
                ...
                self.iters += 1
                self.update_metrics("val", loss.item(), rgb_inputs.size(0),
                                    float((predicted == correct).sum()) / len(correct))
