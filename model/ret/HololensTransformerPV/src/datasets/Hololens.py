...
from datasets.utils.normalize import video_min_max_norm, frame_min_max_norm, video_std, frame_std, video_img_net_std, frame_img_net_std

from pathlib import Path

class Hololens(Dataset):
    """Hololens2 Dataset class"""
    def __init__(self, configer, normal_type, specific_path, width, height, path, crop_path, split="train", data_type="rgb", transforms=None, n_frames=40, optical_flow=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.normal_type=='video_normal':
            ...
            data=frame_std(data)
        elif self.normal_type == 'video_imgnet_stand':
            data=video_img_net_std(data)
        elif self.normal_type == 'frame_imgnet_stand':
            data=frame_img_net_std(data)
